	...
	_peerJoining(peer, returning = false) {
		this._queue.push(async () => {
			...
			if (returning) {
				...
				this._notification(peer.socket, 'roomReady', { turnServers, rtcStatsOptions: config.rtcStatsOptions });
	...
	async _handleSocketRequest(peer, request, cb) {
		...
		switch (request.method) {
			...
			case 'produce': {
				...
				if (
					(appData.source === 'mic' || appData.source === 'screenaudio') &&
			...
			case 'offer': {
				const { offer, peerId } = request.data;

				const receiver = this._peers[peerId];

				if (!receiver)
					throw new Error(`peer with id "${peerId}" not found`);

				this._notification(receiver.socket, 'offer', {
					peerId: peer.id,
					offer
				});
			...
			case 'answer': {
				const { answer, peerId } = request.data;
				...
				this._notification(receiver.socket, 'answer', {
					peerId: peer.id,
					answer
				});
			...
			case 'candidate': {
				const { candidate, peerId } = request.data;
				...
				this._notification(receiver.socket, 'candidate', {
					peerId: peer.id,
					candidate
				});
